...
## 사용 (스니펫 인식 — 기본 흐름, 런타임 AI 없음)
1. Figma에서 화면 프레임 하나 선택
2. **[영역 분석]** → 플러그인이 아이콘/벡터 노이즈를 제거하고 화면을 영역으로 나눠, 각 영역의 스니펫 타입을 1차 추측해 목록으로 보여줌(확실/추측/불명 배지)
3. 영역별 **타입 드롭다운에서 오인식만 수정** (사람이 주는 "한 비트")
4. **[페이지 생성]** → 깔끔한 WebSquare 페이지(`sub_contents` + 스니펫 + `meta_snippet*`) 생성 → [복사] / [XML 파일로 저장]

휴리스틱 인식은 완벽하지 않아도 되며(사람이 타입 수정), 마크업은 규칙이 항상 정확히 낸다. **런타임 AI 호출 0 — zip Import만으로 누구나 사용.**

> 고급: 단일 스니펫/풀페이지(절대좌표) 변환은 하단 "고급" 섹션에서.
